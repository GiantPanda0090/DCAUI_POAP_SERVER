#!/bin/env python3
#md5sum="0126fda2eba00df62357d8a70bbbe54b"
"""
...
def set_defaults_and_validate_options():
    ...
    set_default("use_nxos_boot", True)
...
def install_images_7_x():
    ...
    if (not options['bypass_bios']):
        poap_log("Installing new BIOS (will take up to 5 minutes. Don't abort)")
        install_bios()
    else:
        poap_log("bypass BIOS installation")

    poap_log("Setting Boot Variable to Bypass POAP next time.")
    ...
    try:
        ...
        poap_log(str(clip("dir bootflash:")))
...
def is_bios_upgrade_needed():
    ...
    if re.match("nxos.", options["target_system_image"]):
        ...
        try:
            ...
            poap_log(bios_number)
            poap_log(last_upgrade_bios)
